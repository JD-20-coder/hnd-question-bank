...
router.get('/questions/search', async (req, res) => {
    try {
        const { q = '', field = '', type = '', difficulty = '', limit = '50', offset = '0' } = req.query;
        let query = `
...
router.get('/banks/:id', async (req, res) => {
    ...
    `, [Number(id)]);
        ...
        const [questions] = await db_1.pool.execute(`
...
router.get('/stats', async (req, res) => {
    try {
        const [totalBanks] = await db_1.pool.execute('SELECT COUNT(*) as count FROM question_banks WHERE is_public = true');
        const [totalQuestions] = await db_1.pool.execute('SELECT COUNT(*) as count FROM questions');
        const [totalUsers] = await db_1.pool.execute('SELECT COUNT(*) as count FROM users');
        const [fields] = await db_1.pool.execute('SELECT COUNT(DISTINCT field_of_study) as count FROM question_banks WHERE is_public = true');
        res.json({
            banks: totalBanks[0].count,
